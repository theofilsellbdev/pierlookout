...
const accordionItems = [
  ...
  {
    title: "Parking",
    content: ["Free parking", "Off-site parking"],
  },
  {
...
export default function InformationSection(): JSX.Element {
  ...
  return (
    ...
    >
      <h2 className="text-[clamp(1rem,2vw,2rem)] max-w-6xl mb-4">
        Information
      </h2>
      {/* Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_2fr] h-full w-full max-w-5xl gap-x-[4rem] pb-[4rem] px-[2rem]">
        ...
        <div className="w-full h-full flex flex-col">
          {/* About the flat: */}
          <h3 className="text-[clamp(.7rem,2vw,1.2rem)] mb-1">
            About the flat:
          </h3>
          <p className="text-[clamp(.65rem,2vw,1rem)]">
            Pier Lookout is a tranquil top-floor apartment with sweeping sea
            views over Eastbourne’s historic pier. Just steps from the beach, it
            blends coastal charm with modern comfort. The bright open-plan
            living space is perfect for slow mornings or quiet evenings.
          </p>
          {/* Divider */}
          ...
          <ul className="list-disc list-inside text-[clamp(.65rem,2vw,1rem)]">
            <li>
              Unfortunately the flat is not suitable for children, pets, and
              stag or hen parties.
            </li>
        ...
        <div className="w-full h-full flex flex-col">
          ...
          {/* Getting Here: */}
          <h3 className="text-[clamp(.7rem,2vw,1.2rem)] mb-4">
            Getting Here:
          </h3>
          {/* Map */}
          ...
          <p className="text-[clamp(.65rem,2vw,1rem)] text-end">
          ...
          <div className="w-full h-[1px] bg-gray-300 my-4" />
          {/* Accordion Menus */}
          <AccordianGroup items={accordionItems} />
        </div>
      </div>
    </section>
  );
...
export function AccordianGroup({
  ...
  const handleToggle = (idx: number, title: string) => {
    setOpenIndex((current) => {
      const isOpening = current !== idx;

      trackEvent("information_accordion_toggle", {
        title,
        opened: isOpening,
      });
    ...
    <div className="space-y-3">
      {items.map((it, idx) => (
        <AccordianItem
          ...
          onToggle={() => handleToggle(idx, it.title)}
        />
      ))}
    </div>
  );
